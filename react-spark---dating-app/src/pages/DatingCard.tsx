    ...
    >
      <div className="relative w-full h-full rounded-2xl shadow-2xl overflow-hidden bg-slate-200">
        ...
        <div className="absolute bottom-0 inset-x-0 z-20">
          <div className="h-24 bg-gradient-to-t from-black/80 to-transparent pointer-events-none"></div>
          
          <div 
            onClick={isTopCard ? () => onViewProfile(profile) : undefined}
            className={`p-5 pt-2 bg-black/50 backdrop-blur-md text-white ${isTopCard ? 'cursor-pointer' : 'cursor-default'}`}
            role={isTopCard ? 'button' : undefined}
            aria-label="View profile details"
          >
              {profile.vibe && (
                <div className="mb-2 inline-block px-3 py-1 text-sm bg-white/25 text-white rounded-full font-semibold">
                    {profile.vibe}
                </div>
              )}
              ...
              {profile.distance < 9999 && (
                 <p className="text-sm text-white/90 font-medium">{profile.distance.toFixed(1)} miles away</p>
              )}

              <p className="text-white/90 mt-2 text-base line-clamp-2">{profile.bio}</p>
              
              {profile.interests.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {profile.interests.slice(0, 3).map(interest => (
                    <div key={interest} className="px-3 py-1 text-sm bg-white/20 text-white rounded-full">
                      {interest}
                    </div>
                  ))}
                </div>
          ...
          <>
            ...
            >
             ...
             <motion.div
              ...
              className="absolute bottom-1/2 left-1/2 -translate-x-1/2 text-[#FFC107] font-bold text-4xl border-4 border-[#FFC107] rounded-lg px-4 py-1 bg-white/80 z-10"
            >
              SUPER LIKE
            </motion.div>
          </>
